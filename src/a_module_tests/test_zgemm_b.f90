...
program test_zgemm_b
    ...
    complex*16, allocatable, dimension(:, :) :: AB_orig, BB_orig, CB_orig
...
!$omp end target data
    ...
    if (gen .eq. 1) then
        ...
        N_ops = N_ops*N_ops
        N_ops = 8*s*N_ops
        N_ops = N_ops + 12*s*s ! First zgemm
        N_ops = N_ops*2 ! There are actually two zgemms
        N_ops = N_ops + s*s ! Plus one scale
        if (first .eq. 'T' .or. second .eq. 'T') then
            N_ops = N_ops + 2*s*s ! Plus two conj, here movs, swaps are ops
        end if
        if (first .ne. second) then
            N_ops = N_ops + 2*s*s ! Plus another two conj
        end if
        N_ops = N_ops*repeats ! Times number od repeats

        write (str_tmp1, "(I5)") repeats
        str_tmp1 = adjustl(str_tmp1)
        write (str_tmp2, *) "zgemm_b ", trim(str_tmp1), "x", first, second
        write (str_tmp1, "(I5)") s
        str_tmp1 = adjustl(str_tmp1)
    ...
    deallocate (A, B, AB, BB, CB, CB_i, AB_orig, BB_orig, CB_orig, helper_r, helper_c)

end program test_zgemm_b
